...
export async function GET(request) {
  try {
    ...
    try {
      ...
      console.warn("[client/collection-products] client API unreachable, using fallback:", err.message);
    }
    ...
    if (products.length === 0) {
      ...
      products = FALLBACK_PRODUCTS.filter(p =>
        ...
        (collectionId && p.id === Number(collectionId))
      );
    }
    ...
    return Response.json({ success: true, collection: null, count: 0, products: [] });
  }
}
